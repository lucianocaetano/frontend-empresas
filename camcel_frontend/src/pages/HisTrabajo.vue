...
const filterItems = () => {
    if (selectedOption.value === 'Todos') {
        item.value = itemTemp.value; // Muestra todos los trabajos
    } else if (selectedOption.value === 'Aceptados') {
        // Filtra los trabajos donde confirmacionPREV es 1 y confirmacionEmpresa es 1 (o true)
        item.value = itemTemp.value.filter(job => job.confirmacionPREV === 1 && job.confirmacionEmpresa === 1);
        console.log('Trabajos aceptados:', item.value);
    } else if (selectedOption.value === 'Rechazados') {
        // Filtra los trabajos donde confirmacionPREV es 0 (o false)
        item.value = itemTemp.value.filter(job => job.confirmacionPREV === 0 || job.confirmacionEmpresa === 0);
    }
};
